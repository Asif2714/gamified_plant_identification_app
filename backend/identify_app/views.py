...
@csrf_exempt  # For simplicity, disabling CSRF. For production, consider using CSRF tokens.
def upload_image(request):
    print(request.body)
    if request.method == 'POST':
        # image_file = request.FILES.get('image', None)
        data = request.body.decode('utf-8')  # Decode the raw bytes to a string
...
#                     else:
#                         return JsonResponse({'error': 'Predicted class not in JSON'}, status=404)
#                 else:
#                     return JsonResponse({'error': 'Confidence too low'}, status=200)
...
def predict_image(request):
    ...
    # print("Full request data:", request)
